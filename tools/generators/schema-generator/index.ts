...
export default async function (tree: Tree, schema: any) {
  if (schema.name !== 'schema') {
    throw new Error('We only support use this command for @shukun/schema library.');
  }
...
function generate (treeRoot: string, projectRoot: string) {
  ...
  const queue = list.map(async (fileName: string) => {
    const ts = await compileFromFile(
      join(treeRoot, projectRoot, 'src', originFolder, `${fileName}.schema.json`),
      {
  ...
  Promise.all(queue).then(() => console.log("Generate type files successfully."));
}